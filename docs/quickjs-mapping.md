...
| Bytecode compiler | `crates/bytecode` | In Progress | Script compilation, function table, call/return ops, scope ops, unary/comparison ops, jump ops (`JumpIfFalse`/`Jump`), loop/switch control patching (`break/continue`), and exception-handler ops (`throw`/`try`) landed. |
...
- Complete completion-record semantics through `finally` (`return`/`break`/`continue` interaction).
