...
- 已完成：
  ...
  - `annexB` 全目录：`806/19`（`target/test262-annexb-baseline-20260225-v7.json`，`global-code` 已清零，`function-code` 仅剩 `4` 条，整体剩余失败簇主要在 `assignmenttargettype` 与 parse negative 角落）
...
- 下一轮并行模块拆分（建议 4 条线并行，每线 2~3h）：
  ...
  - Track L（Annex B AssignmentTargetType）：在 eval/function/global block function declaration 清零后，继续对齐 `annexB/language/expressions/assignmenttargettype` 的 early-error / runtime throw 分支。
- 每个 Track 必须输出：
  - QuickJS 对照点（函数名/分支）
  - 代码提交（最小可验证增量）
