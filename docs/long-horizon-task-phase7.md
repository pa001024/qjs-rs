...
- 已完成：
  ...
  - `annexB` 全目录：`684/141`（`target/test262-annexb-baseline-20260225-v5.json`，失败簇主要集中在 `annexB/language/function-code` 与 `annexB/language/global-code`）
...
- 下一轮并行模块拆分（建议 4 条线并行，每线 2~3h）：
  ...
  - Track L（Annex B Function/Global 后续）：在 `annexB/language/eval-code/(direct|indirect)` 清零后，继续对齐 `annexB/language/function-code` 与 `annexB/language/global-code` 的 block-level function declaration 绑定语义失败簇。
- 每个 Track 必须输出：
  - QuickJS 对照点（函数名/分支）
  - 代码提交（最小可验证增量）
