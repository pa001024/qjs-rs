...
- `language max-cases=5000`: discovered=23882, executed=1585, skipped=22297, passed=1328, failed=257
...
- runner 新增 `*_FIXTURE.js` 跳过策略（这些文件为 test262 支撑脚本，不作为独立测试执行），因此 `executed/skipped` 与旧快照不可直接逐项对比；在该口径下当前 `failed` 进一步下降到 `257`。
