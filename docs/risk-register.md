...
| R-010 | `finally` completion behavior is now baseline for `return`/`break`/`continue`, but may still diverge in deep edge cases vs spec completion records. | Medium | Validate against broader test262 coverage and move to explicit completion records only if gaps are confirmed. | In Progress |
