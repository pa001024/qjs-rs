...
| R-010 | `finally` currently does not fully model completion-record precedence for `return`/`break`/`continue`. | High | Introduce explicit completion records in bytecode/VM before strict conformance push. | Open |
