...
- `test262 annexB` 最新全量：`executed=825`、`passed=806`、`failed=19`（快照：`target/test262-annexb-baseline-20260225-v7.json`）。
...
- 本轮新增语义收敛：
  ...
  - Annex B function/global block function declaration 再收敛：VM 将同一套 varEnv 同步框架扩展到 non-strict script/function 执行路径，结合“跳过 catch BindingIdentifier 词法冲突判定”和“参数名排除”规则，对齐 QuickJS `create_func_var/OP_scope_put_var` 行为；`annexB` 进一步提升到 `806/19`，`function-code/global-code` 主簇已清理，剩余失败集中在 `assignmenttargettype`、少量 parse negative 与对象属性边界。
