...
- `test262 annexB` 最新全量：`executed=825`、`passed=825`、`failed=0`（快照：`target/test262-annexb-baseline-20260226-v14.log`）。
- `test262 built-ins/WeakMap` 最新全量：`discovered=141`、`executed=37`、`passed=37`、`failed=0`。
- `test262 built-ins/WeakSet` 最新全量：`discovered=85`、`executed=41`、`passed=41`、`failed=0`。
- `test262 built-ins/Proxy` 最新全量：`discovered=311`、`executed=1`、`passed=1`、`failed=0`（当前可执行样例已清零）。
- `test262 built-ins/Map --max-cases 200` 最新采样：`discovered=204`、`executed=53`、`passed=53`、`failed=0`（快照：`target/test262-builtins-map-20260226-v2-s200.json`，阶段对比：`v1-s200=49/4` -> `v2-s200=53/0`）。
- `test262 built-ins/Set --max-cases 200` 最新采样：`discovered=383`、`executed=126`、`passed=126`、`failed=0`（快照：`target/test262-builtins-set-20260226-v2-s200.json`，阶段对比：`v1-s200=123/3` -> `v2-s200=126/0`）。
- `test262 built-ins/Boolean` 最新全量：`discovered=51`、`executed=39`、`passed=39`、`failed=0`（快照：`target/test262-builtins-boolean-20260226-v2.json`，阶段对比：`v1=34/5` -> `v2=39/0`）。
- `test262 built-ins --max-cases 5000` 最新采样：`executed=5000`、`passed=4405`、`failed=595`（快照：`target/test262-builtins-baseline-20260226-v3-s5000.json`，阶段对比：`v2-s5000=4400/600` -> `v3-s5000=4405/595`）。
...
- 本轮新增语义收敛：
  ...
  - Annex B 尾部簇清零（QuickJS 对照 + 语义补齐）：parser/bytecode/vm 新增 call-expression assignment target 的 runtime `ReferenceError` 路径、补齐 `for (var x = init in ...)` non-strict initializer、放宽 non-strict block/switch 中 FunctionDeclaration 重名早错（B.3.3.4/B.3.3.5）；regexp 侧切换 `fancy-regex` 执行后端并补齐 legacy escape/quantifiable assertion/non-empty class range fallback 归一化。`annexB` 最终收敛到 `825/0`（`target/test262-annexb-baseline-20260226-v14.log`）。
  - built-ins/WeakMap/WeakSet/Proxy 收敛：对齐 QuickJS `js_map_constructor` 新目标检查与 iterable 初始化流程（`Get(set/add)`、`IsCallable`、`GetIterator`），补齐 `Set.prototype.delete/has` 最小语义，并接入 `Proxy.revocable`/`revoke`（含 revoke 函数 `length/name` 属性顺序），对应三组子集均清零。
  - built-ins/Map/Set 迭代与 size 收敛：对齐 QuickJS `js_map_delete/js_map_clear/js_map_forEach/js_map_iterator_next/js_map_get_size` 路径，Map/Set 内部存储切换为“保留 tombstone 槽位 + live iterator 视图”，补齐 `delete/clear` 对迭代器稳定性、`forEach` 在 delete+re-add 场景下重访新条目、`Set.prototype.keys === Set.prototype.values` 与 `Map/Set.prototype.size` brand check（`Type(M) is Object` + internal slot 要求），对应 `built-ins/Map --max-cases 200` 与 `built-ins/Set --max-cases 200` 均收敛至 `0` 失败。
  - built-ins/Boolean 收敛：对齐 QuickJS `Boolean.prototype` 初值语义（`[[BooleanData]] = false`），为 `Boolean.prototype` 挂接 boxed primitive internal slot，修复 `Boolean.prototype.toString/valueOf` 在 prototype receiver 下误抛 `TypeError`；`built-ins/Boolean` 由 `34/5` 提升到 `39/0`。
