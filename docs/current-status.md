...
- `test262 annexB/language/eval-code/direct` 最新全量：`executed=276`、`passed=169`、`failed=107`（快照：`target/test262-annexb-language-eval-code-direct-20260225-v3.json`）。
...
- 本轮新增语义收敛：
  ...
  - Annex B eval direct 语法入口收敛：parser 放开语句位函数声明（`if/while/for/switch` 的 embedded statement），对齐 QuickJS Annex B 解析入口；`annexB/language/eval-code/direct` 失败簇从“语法拒绝”转向“运行时绑定语义”（当前 `169/107`），为后续 B.3.3 var/binding 语义实现建立可执行基线。
