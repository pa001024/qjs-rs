...
impl Vm {
    ...
    fn enqueue_host_function_values(&mut self, host: HostFunction) {
        match host {
            ...
            | HostFunction::ArrayCopyWithinThis
    ...
    fn is_concat_spreadable_value(
        ...
        let spreadable =
            self.get_property_from_receiver(value.clone(), "Symbol.isConcatSpreadable", realm)?;
        if !matches!(spreadable, JsValue::Undefined) {
            return Ok(self.is_truthy(&spreadable));
        }
        match value {
            JsValue::Object(object_id) => {
    ...
    fn array_species_create_for_concat(
        ...
        if !Self::is_object_like_value(&constructor) {
            return Err(VmError::TypeError(
                "Array.prototype.concat constructor must be object",
            ));
        }
        let species =
            self.get_property_from_receiver(constructor.clone(), "Symbol.species", realm)?;
        let constructor = if matches!(species, JsValue::Undefined | JsValue::Null) {
            constructor
        } else {
            species
        };
        ...
        if !is_constructor {
            return Err(VmError::TypeError(
                "Array.prototype.concat species must be constructor",
            ));
        }
        let result = self.execute_construct_value(
            constructor,
        ...
        if !Self::is_object_like_value(&result) {
            return Err(VmError::TypeError(
                "Array.prototype.concat species must create object",
            ));
        }
        Ok(result)
    }

    fn execute_array_concat_this(
        &mut self,
        this_arg: Option<JsValue>,
        args: &[JsValue],
        realm: &Realm,
        ...
        let source = self.object_prototype_this_object(
            this_arg,
        ...
        for candidate in std::iter::once(source).chain(args.iter().cloned()) {
            if self.is_concat_spreadable_value(&candidate, realm)? {
                let length = self.array_like_length_from_value(&candidate, realm)?;
                for element_index in 0..length {
                    let key = element_index.to_string();
                    if self.has_property_on_receiver(&candidate, &key, realm)? {
                        let value =
                            self.get_property_from_receiver(candidate.clone(), &key, realm)?;
                        self.create_data_property_or_throw(
                            result.clone(),
                            index.to_string(),
                            value,
    ...
    fn execute_array_every_this(
        ...
        if !Self::is_callable_value(&callback) {
            return Err(VmError::TypeError(
                "Array.prototype.every callback must be callable",
            ));
        }
    ...
    fn execute_array_for_each_this(
        ...
        if !Self::is_callable_value(&callback) {
            return Err(VmError::TypeError(
                "Array.prototype.forEach callback must be callable",
            ));
        }
        let callback_this = args.get(1).cloned().unwrap_or(JsValue::Undefined);
        for index in 0..length {
    ...
    fn execute_array_find_this(
        ...
        if !Self::is_callable_value(&callback) {
            return Err(VmError::TypeError(
                "Array.prototype.find/findIndex callback must be callable",
            ));
        }
        let callback_this = args.get(1).cloned().unwrap_or(JsValue::Undefined);
        for index in 0..length {
    ...
    fn execute_array_filter_this(
        ...
        if !Self::is_callable_value(&callback) {
            return Err(VmError::TypeError(
                "Array.prototype.filter callback must be callable",
            ));
        }
        let callback_this = args.get(1).cloned().unwrap_or(JsValue::Undefined);
        let result =
    ...
    fn execute_array_fill_this(
        ...
        let end_index = match args.get(2) {
            Some(JsValue::Undefined) | None => length as f64,
            Some(value) => to_integer_or_infinity(self.coerce_number_runtime(
                value.clone(),
                realm,
    ...
    fn execute_array_copy_within_this(
        &mut self,
        this_arg: Option<JsValue>,
        args: &[JsValue],
        realm: &Realm,
        ...
        let to = normalize_index(to_index, length);
        let from = normalize_index(from_index, length);
        let final_index = normalize_index(end_index, length);
        ...
        let mut from_cursor = if direction < 0 {
            from + count - 1
        } else {
            from
        };
        let mut to_cursor = if direction < 0 { to + count - 1 } else { to };

        while count > 0 {
            let from_key = from_cursor.to_string();
            let to_key = to_cursor.to_string();
            if self.has_property_on_receiver(&target, &from_key, realm)? {
                let value = self.get_property_from_receiver(target.clone(), &from_key, realm)?;
                let _ =
                    self.set_property_on_receiver(target.clone(), to_key.clone(), value, realm)?;
            } else {
                let deleted = self.delete_property(target.clone(), to_key.clone())?;
                if !deleted {
                    return Err(VmError::TypeError(
                        "Array.prototype.copyWithin cannot delete target property",
                    ));
                }
            }

            from_cursor += direction;
            to_cursor += direction;
            count -= 1;
        }
    ...
    fn execute_host_function_call(
        ...
        match host {
            ...
            HostFunction::ArrayJoinThis => {
                ...
                for index in 0..length {
                    ...
                    let part = match value {
                        ...
                        other => self.coerce_to_string_runtime(other, realm, caller_strict)?,
                    };
                    parts.push(part);
                }
            ...
            HostFunction::ArrayLastIndexOfThis => {
                self.execute_array_index_lookup(this_arg, &args, realm, true, caller_strict)
            }
            HostFunction::ArrayFrom => {
            ...
            HostFunction::ArrayFindThis => {
                self.execute_array_find_this(this_arg, &args, realm, caller_strict, false)
            }
            HostFunction::ArrayFindIndexThis => {
                self.execute_array_find_this(this_arg, &args, realm, caller_strict, true)
            }
            HostFunction::ArrayCopyWithinThis => {
                self.execute_array_copy_within_this(this_arg, &args, realm, caller_strict)
            }
            HostFunction::ArrayConcatThis => {
                self.execute_array_concat_this(this_arg, &args, realm, caller_strict)
            }
    ...
    fn execute_array_iterator_next(
        ...
        if index >= length {
            if let Some(iterator) = self.objects.get_mut(&iterator_id) {
                iterator
                    .properties
                    .insert(ARRAY_ITERATOR_TARGET_KEY.to_string(), JsValue::Undefined);
            }
    ...
    fn execute_object_define_property(
        ...
        let target = match args.first() {
            ...
            Some(JsValue::NativeFunction(native)) => DefinePropertyTarget::NativeFunction(*native),
            _ => {
                return Err(VmError::TypeError(
        ...
        let mut pending_array_length_update: Option<(JsValue, Option<bool>)> = None;
        {
            ...
            if has_get || has_set {
                ...
                let defines_data = has_value || desc_writable.is_some();
                if defines_data {
    ...
    fn execute_object_for_in_keys(&mut self, args: &[JsValue]) -> Result<JsValue, VmError> {
        ...
        let keys = match target {
            value if Self::is_object_like_value(&value) => self.collect_for_in_keys(value)?,
    ...
    fn collect_for_in_keys(&mut self, start: JsValue) -> Result<Vec<String>, VmError> {
        let mut keys = Vec::new();
        let mut seen = BTreeSet::new();
        let mut current = start;
        while Self::is_object_like_value(&current) {
            let own_keys = self.collect_own_property_keys(&current, false)?;
            for key in own_keys {
                let enumerable = match &current {
                    JsValue::Object(object_id) => self
                        .objects
                        .get(object_id)
                        .and_then(|object| object.property_attributes.get(&key))
                        .map(|attributes| attributes.enumerable)
                        .unwrap_or(true),
                    JsValue::Function(closure_id) => self
                        .closure_objects
                        .get(closure_id)
                    ...
                    JsValue::HostFunction(host_id) => self
                        ...
                        .map(|attributes| attributes.enumerable)
                        .unwrap_or(true),
                    JsValue::NativeFunction(_) => false,
                    _ => false,
                };
    ...
    fn object_proxy_slots(
        &self,
        object_id: ObjectId,
    ) -> Result<Option<(JsValue, JsValue)>, VmError> {
        if !self.has_object_marker(object_id, PROXY_OBJECT_MARKER_KEY)? {
            return Ok(None);
        }
        let object = self
        ...
        match receiver {
            JsValue::Object(object_id) => {
                ...
                let object = self
                    ...
                    .get(object_id)
                    .ok_or(VmError::UnknownObject(*object_id))?;
                if let Some(JsValue::String(value)) =
                    object.properties.get(BOXED_PRIMITIVE_VALUE_KEY)
                {
                    if property == "length" {
                        return Ok(true);
                    }
                    if let Ok(index) = property.parse::<usize>() {
                        return Ok(index < Self::utf16_code_unit_length(value));
                    }
                }
            ...
            JsValue::Function(closure_id) => {
                ...
                let mut current = self.get_prototype_of_value(&JsValue::Function(*closure_id))?;
                while Self::is_object_like_value(&current) {
        ...
        match base {
            ...
            JsValue::NativeFunction(native) => {
                ...
                while Self::is_object_like_value(&current) {
                    if self.has_own_property(&current, property)? {
                        return self.get_property_from_base_with_receiver(
                            current, property, receiver, realm,
                        );
                    }
                    current = self.get_prototype_of_value(&current)?;
                }
    ...
    fn get_object_property_with_receiver(
        ...
        if property == "find" && is_array_like && !is_array_prototype {
            return Ok(self.create_host_function_value(HostFunction::ArrayFindThis));
        }
        if property == "findIndex" && is_array_like && !is_array_prototype {
            return Ok(self.create_host_function_value(HostFunction::ArrayFindIndexThis));
        }
        if property == "copyWithin" && is_array_like && !is_array_prototype {
            return Ok(self.create_host_function_value(HostFunction::ArrayCopyWithinThis));
        }
    ...
    fn math_object_value(&mut self) -> Result<JsValue, VmError> {
        ...
        };
        {
            ...
            math_object.properties.insert(
                "Symbol.toStringTag".to_string(),
                JsValue::String("Math".to_string()),
            );
            math_object.property_attributes.insert(
                "Symbol.toStringTag".to_string(),
                PropertyAttributes {
    ...
    fn json_object_value(&mut self) -> Result<JsValue, VmError> {
        ...
        let parse = self.create_host_function_value(HostFunction::JsonParse);
        {
            ...
            json_object.properties.insert(
                "Symbol.toStringTag".to_string(),
                JsValue::String("JSON".to_string()),
            );
            json_object.property_attributes.insert(
                "Symbol.toStringTag".to_string(),
                PropertyAttributes {
    ...
    fn array_prototype_value(&mut self) -> JsValue {
        ...
        if let JsValue::Object(id) = prototype {
            ...
            if let Some(object) = self.objects.get_mut(&id) {
                ...
                object.properties.insert("fill".to_string(), fill);
                object.property_attributes.insert(
                    "fill".to_string(),
                    PropertyAttributes {
                        writable: true,
                ...
                object.properties.insert("find".to_string(), find);
                object.property_attributes.insert(
                    "find".to_string(),
                    PropertyAttributes {
                        writable: true,
                ...
                object
                    .properties
                    .insert("findIndex".to_string(), find_index);
                object.property_attributes.insert(
                    "findIndex".to_string(),
                    PropertyAttributes {
                        writable: true,
                ...
                object.properties.insert("forEach".to_string(), for_each);
                object.property_attributes.insert(
                    "forEach".to_string(),
                    PropertyAttributes {
                        writable: true,
                ...
                object
                    .properties
                    .insert("copyWithin".to_string(), copy_within);
                object.property_attributes.insert(
        ...
        match value {
            JsValue::Object(object_id) => {
                ...
                if self.objects.get(&object_id).is_some_and(|object| {
                    ...
                    return match self.execute_array_join(object_id, ",")? {
            ...
            JsValue::Function(closure_id) => {
                let primitive = self.ordinary_to_primitive_for_function(
                    closure_id,
                    true,
                    realm,
                    caller_strict,
    ...
    fn set_function_property(
        ...
        if data_writable.is_none() {
            let mut current = self.get_prototype_of_value(&JsValue::Function(closure_id))?;
            while Self::is_object_like_value(&current) {
                let (setter, inherited_getter_exists, inherited_data_writable) = match &current {
                    JsValue::Object(object_id) => {
                        let object = self
                            .objects
                            .get(object_id)
                    ...
                    JsValue::Function(proto_closure_id) => {
                        let object = self.closure_objects.get(proto_closure_id);
                        (
                            object.and_then(|object| object.setters.get(&property).cloned()),
                            object.is_some_and(|object| object.getters.contains_key(&property)),
                    ...
                    JsValue::HostFunction(proto_host_id) => {
                        let object = self.host_function_objects.get(proto_host_id);
                        (
                            object.and_then(|object| object.setters.get(&property).cloned()),
                            object.is_some_and(|object| object.getters.contains_key(&property)),
                            object.and_then(|object| {
                ...
                if inherited_getter_exists {
                    return Ok(value);
                }
                if inherited_data_writable == Some(false) {
                    return Ok(value);
                }
                if inherited_data_writable == Some(true) {
                    break;
                }
                current = self.get_prototype_of_value(&current)?;
            }
    ...
    fn has_own_property(&self, target: &JsValue, property: &str) -> Result<bool, VmError> {
        match target {
            ...
            JsValue::HostFunction(host_id) => {
                ...
                if let Some(object) = self.host_function_objects.get(host_id) {
                    return Ok(object.properties.contains_key(property)
                        || object.getters.contains_key(property)
                        || object.setters.contains_key(property));
                }
    ...
    fn to_length_from_number(number: f64) -> i64 {
        ...
        if number.is_infinite() {
            return MAX_SAFE_INTEGER_F64 as i64;
        }
        number.floor().min(MAX_SAFE_INTEGER_F64) as i64
    }
    ...
    fn array_like_length_from_value(
        ...
        Ok(Self::to_length_from_number(self.to_number(&length_value)))
    }
    ...
    fn get_host_function_property(
        ...
        match property {
            ...
            _ => {
                let prototype = self.get_prototype_of_value(&JsValue::HostFunction(host_id))?;
                if Self::is_object_like_value(&prototype) {
                    self.get_property_from_base_with_receiver(
                        prototype,
                        property,
    ...
    fn get_function_property(
        ...
        match property {
            ...
            _ => {
                let prototype = self.get_prototype_of_value(&JsValue::Function(closure_id))?;
                if Self::is_object_like_value(&prototype) {
                    self.get_property_from_base_with_receiver(
                        prototype,
                        property,
...
mod tests {
    ...
    #[test]
    fn array_concat_handles_holes_and_primitive_receivers() {
        let script = parse_script(
            "Array.prototype[1] = 1; var x = [0]; x.length = 2; var y = x.concat(); var ok = y[0] === 0 && y[1] === 1 && y.hasOwnProperty('1') && (Array.prototype.concat.call(true)[0] instanceof Boolean); delete Array.prototype[1]; ok;",
        )
        .expect("script should parse");
        let chunk = compile_script(&script);
        let mut realm = Realm::default();
    ...
    #[test]
    fn array_copy_within_preserves_holes_and_returns_receiver() {
        let script = parse_script(
            "var arr = [0, 1, , , 1]; var result = arr.copyWithin(0, 1, 4); result === arr && arr.length === 5 && arr[0] === 1 && arr[4] === 1 && !arr.hasOwnProperty(1) && !arr.hasOwnProperty(2) && !arr.hasOwnProperty(3);",
        )
        .expect("script should parse");
        let chunk = compile_script(&script);
        let mut realm = Realm::default();
    ...
    #[test]
    fn array_every_supports_generic_array_like_values() {
        let script = parse_script(
            "var obj = {0: 1, length: 1}; function cb(v, i, o) { return v === 1 && i === 0 && o === obj; } typeof Array.prototype.every === 'function' && Array.prototype.every.call(obj, cb) === true;",
        )
        .expect("script should parse");
        let chunk = compile_script(&script);
        let mut realm = Realm::default();
    ...
    #[test]
    fn array_fill_returns_receiver_and_supports_primitive_this() {
        let script = parse_script(
            "var arr = [0, 1, 2]; var ret = arr.fill(9, 1, 2); var boxed = Array.prototype.fill.call(true); ret === arr && arr[0] === 0 && arr[1] === 9 && arr[2] === 2 && (boxed instanceof Boolean);",
        )
        .expect("script should parse");
        let chunk = compile_script(&script);
        let mut realm = Realm::default();
    ...
    #[test]
    fn array_find_and_find_index_cover_holes() {
        let script = parse_script(
            "var arr = [undefined, , , 'foo']; var called = 0; var idx = arr.findIndex(function(v) { called++; return v === 'foo'; }); called === 4 && idx === 3 && arr.find(function(v) { return v === 'foo'; }) === 'foo';",
        )
        .expect("script should parse");
        let chunk = compile_script(&script);
        let mut realm = Realm::default();
    ...
    #[test]
    fn array_for_each_supports_generic_receivers() {
        let script = parse_script(
            "var obj = {0: 1, 2: 3, length: 3}; var sum = 0; Array.prototype.forEach.call(obj, function(v) { sum += v; }); sum === 4;",
        )
        .expect("script should parse");
        let chunk = compile_script(&script);
        let mut realm = Realm::default();
    ...
    #[test]
    fn array_entries_iterator_stays_done_after_exhaustion() {
        let script = parse_script(
            "var array = []; var iterator = array.entries(); array.push('a'); var first = iterator.next(); var exhausted = iterator.next(); array.push('b'); var afterPush = iterator.next(); first.done === false && first.value[0] === 0 && first.value[1] === 'a' && exhausted.done === true && exhausted.value === undefined && afterPush.done === true && afterPush.value === undefined;",
        )
        .expect("script should parse");
        let chunk = compile_script(&script);
        let mut realm = Realm::default();
