...
- [ ] **PERF-03**: `qjs-rs` aggregate mean latency on the tracked benchmark suite is **at most `1.25x quickjs-c`** on the same host and run configuration (equivalent to **>=80% of `quickjs-c` performance**). _(Open gap: latest authoritative packet-i artifact `target/benchmarks/engine-comparison.local-dev.packet-i.json` (`2026-03-03T09:53:57.185Z`) is still above threshold (`qjs-rs/quickjs-c = 6.345517x`); machine-checkable verdict bundle: `target/benchmarks/phase11-closure-bundle.packet-i.json`; active closure checker mode is `--require-qjs-lte-quickjs-ratio 1.25`.)_
- [ ] **PERF-04**: At least two identified runtime hot paths receive targeted optimization backed by before/after benchmark evidence. _(Implemented with packets A/B/C/D/E/F/final/g/h/i plus parity/hotspot evidence; packet-i authoritative bundle is contract-valid, but milestone closure remains open until PERF-03 passes.)_
- [ ] **PERF-05**: Optimization changes avoid large architectural regressions and preserve maintainability boundaries (no runtime-core C FFI). _(Boundary evidence remains positive in historical packet runs and stays explicitly tracked in the packet-i closure cycle; requirement remains open while PERF-03 is unresolved.)_
...
| PERF-03 | Phase 11 | Open (gap: latest authoritative packet-i quickjs-ratio is `6.345517x`, above `1.25x`) |
| PERF-04 | Phase 11 | Open (packet evidence landed through packet-i; phase closure still blocked by PERF-03) |
| PERF-05 | Phase 11 | Open (maintainability boundary evidence is retained in packet-i closure bundle; phase closure still blocked by PERF-03) |
